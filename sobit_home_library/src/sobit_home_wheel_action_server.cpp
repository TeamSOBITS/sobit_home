  ...
  const std::shared_ptr<GoalHandleMoveWheelLinear> goal_handle)
{
  ...
  while (curt_dist < goal_dist) {
    ...
    // PID
    if ( goal_dist <= 0.1 ) {
      vel_linear =  wheel_linear_kp_ * ( goal_dist + 0.001 - curt_dist )
                  - wheel_linear_kd_ * vel_diff
                  + wheel_linear_ki_ * ( goal_dist + 0.001 - curt_dist ) * std::pow( elapsed_time, 2 );
    } else {
      vel_linear =  wheel_linear_kp_ * ( goal_dist + 0.001 - curt_dist )
                  - wheel_linear_kd_ * vel_diff
                  + wheel_linear_ki_ * ( goal_dist + 0.001 - curt_dist ) * std::pow( elapsed_time, 2 ) / 8.0 * goal_dist;
    }
  ...
  const std::shared_ptr<GoalHandleMoveWheelRotate> goal_handle)
{
  ...
  while (moved_angle < goal_angle) {
    ...
    if (goal_angle_deg < 30) {
      vel_angular = wheel_rotate_kp_ * (goal_angle + 0.001 - moved_angle)
                  - wheel_rotate_kd_ * vel_diff
                  + wheel_rotate_ki_ * (goal_angle + 0.001 - moved_angle) * pow(elapsed_time, 2);
    } else {
      vel_angular = wheel_rotate_kp_ * (goal_angle + 0.001 - moved_angle)
                  - wheel_rotate_kd_ * vel_diff
                  + wheel_rotate_ki_ * (goal_angle + 0.001 - moved_angle) * pow(elapsed_time, 2) * (22.5 / goal_angle);
    }
