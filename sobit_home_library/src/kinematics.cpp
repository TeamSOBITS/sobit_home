...
namespace sobit_home
{
    ...
    bool is_right)
  {
    ...
    if (target_z >= 0.0) {
      ...
      const double theta1 = -std::acos(std::clamp(cos_v, -1.0, 1.0))
                            + std::atan2(vert_dist, horiz_dist);
      const double theta2 = std::atan2(
        vert_dist  - L1 * std::sin(theta1),
        horiz_dist - L1 * std::cos(theta1)) - theta1;
